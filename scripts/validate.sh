...
run_stage "$PYTHON" -B scripts/project_checks.py architecture "${project_args[@]}" || exit "$status"
run_stage "$PYTHON" -B scripts/project_checks.py documentation "${project_args[@]}" || exit "$status"
...
if [ "$full" -eq 1 ]; then
  ...
  run_stage "$PYTHON" -B scripts/project_checks.py architecture --release "${project_args[@]}" || exit "$status"
  run_stage "$PYTHON" -B scripts/project_checks.py documentation --release "${project_args[@]}" || exit "$status"
fi
